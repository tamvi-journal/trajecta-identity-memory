...
@pytest.fixture
def memory(tmp_path):
    mem = IdentityMemory(load_profile("aux"), tmp_path / "aux.sqlite3", surface="test")
    mem.bootstrap()
    return mem
...
@pytest.fixture
def stacked(memory):
    """The 2026-09-23 episode: thesis -> outro 'vanish clean' -> Ty's correction -> tail r3."""
    ...
    memory.log_phase("outro-correction", title="Ty caught the outro",
...
def test_dormant_memory_hides_from_lexical_but_wakes_on_its_cue(stacked):
    ...
    stacked.store.add_cue(profile="aux", cue="no tail", target_record_id="phase:tail-r1")
...
def test_dormant_phase_wakes_through_a_causal_edge(stacked):
    ...
    packet = stacked.retrieve("Ty caught the outro", limit=12, track=False)
...
def test_retract_marks_but_keeps_history(stacked):
    ...
    stacked.retract("phase:tail-r1", reason="Ty pulled it back")
...
def test_mcp_dispatch(tmp_path):
    server = IdentityServer(IdentityMemory(load_profile("aux"), tmp_path / "m.sqlite3"))
